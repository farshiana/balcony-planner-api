...
describe('Plants POST', () => {
    ...
    beforeAll(async (done) => {
        ({ cookie } = await auth({ username: 'plants.post' }));
        const genus = await createGenus({ name: 'plants.post' });
        variety = await createVariety({ name: 'plants.post', genusId: genus.id });
        done();
    });

    let params;
    beforeEach(() => {
        params = { varietyId: variety.id, notes: faker.random.word() };
    });
